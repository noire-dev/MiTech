...
{ 
 int utf32 = 0; 
 unsigned char cur_char = *c;
 
 if (cur_char >> 7 == 0) {
  utf32 = cur_char;
  c++;
 } else if (cur_char >> 5 == 0b110) {
  utf32 |= (cur_char & 0b11111) << 6;
  utf32 |= (*(c + 1)) & 0b111111;
  c += 2;
 } else if (cur_char >> 4 == 0b1110) {
  utf32 |= (cur_char & 0b1111) << 12;
  utf32 |= ((*(c + 1)) & 0b111111) << 6;
  utf32 |= (*(c + 2)) & 0b111111;
  c += 3;
 } else if (cur_char >> 3 == 0b11110) {
  utf32 |= (cur_char & 0b111) << 18;
  utf32 |= ((*(c + 1)) & 0b111111) << 12;
  utf32 |= ((*(c + 2)) & 0b111111) << 6;
  utf32 |= (*(c + 3)) & 0b111111;
  c += 4;
 } else {
  Com_DPrintf("Unrecognised UTF-8 character: 0x%x\n", (unsigned int)*c);
  c++;
 }
