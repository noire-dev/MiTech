...
/*
################
Noire.dev - R_BoxSurfaces2_r
################
*/

static void R_BoxSurfaces2_r(mnode_t *node, vec3_t mins, vec3_t maxs, surfaceType_t **list, int listsize, int *listlength, vec3_t dir) {
	...
	if (node->contents == CONTENTS_NODE) {
		...
		if (s == 1) {
			...
			// Work with
			R_BoxSurfaces2_r(node->children[0], mins, maxs, list, listsize, listlength, dir);
			node = node->children[1];
		}
	}
	...
	if (node->nummarksurfaces > 0) {
		surf = node->firstmarksurface[0];  // Surface found

		// Print it
		if (surf->shader) {
			ri.Printf( PRINT_ALL, "Shader Name: %s\n", surf->shader->name);
		}
	}
}
