...
let GL = {
  ...
  generateTempBuffers: function (quads, context) {
    let largestIndex = GL.log2ceilLookup(GL.MAX_TEMP_BUFFER_SIZE);
    ...
    for (let i = 0; i <= largestIndex; ++i) {
      ...
      let ringbufferLength = GL.numTempVertexBuffersPerSize;
      context.tempVertexBuffers1[i] = [];
      context.tempVertexBuffers2[i] = [];
      let ringbuffer1 = context.tempVertexBuffers1[i];
      let ringbuffer2 = context.tempVertexBuffers2[i];
    ...
    if (quads) {
      ...
      let numIndexes = GL.MAX_TEMP_BUFFER_SIZE >> 1;
      let quadIndexes = new Uint16Array(numIndexes);
      let i = 0, v = 0;
      while (1) {
        quadIndexes[i++] = v;
  ...
  getTempVertexBuffer: function getTempVertexBuffer(sizeBytes) {
    let idx = GL.log2ceilLookup(sizeBytes);
    let ringbuffer = GL.currentContext.tempVertexBuffers1[idx];
    let nextFreeBufferIndex = GL.currentContext.tempVertexBufferCounters1[idx];
    GL.currentContext.tempVertexBufferCounters1[idx] = GL.currentContext.tempVertexBufferCounters1[idx] + 1 & GL.numTempVertexBuffersPerSize - 1;
    let vbo = ringbuffer[nextFreeBufferIndex];
    if (vbo) {
      return vbo;
    }
    let prevVBO = GLctx.getParameter(34964);
  ...
  getTempIndexBuffer: function getTempIndexBuffer(sizeBytes) {
    let idx = GL.log2ceilLookup(sizeBytes);
    let ibo = GL.currentContext.tempIndexBuffers[idx];
    if (ibo) {
      return ibo;
    }
    let prevIBO = GLctx.getParameter(34965);
  ...
  getSource: function (shader, count, string, length) {
    ...
    for (let i = 0; i < count; ++i) {
      let len = length ? SAFE_HEAP_LOAD(length + i * 4 | 0, 4, 0) | 0 : -1;
  ...
  calcBufLength: function calcBufLength(size, type, stride, count) {
    ...
    let typeSize = GL.byteSizeByType[type - GL.byteSizeByTypeRoot];
  ...
  preDrawHandleClientVertexAttribBindings: function preDrawHandleClientVertexAttribBindings(count) {
    ...
    for (let i = 0; i < GL.currentContext.maxVertexAttribs; ++i) {
      let cb = GL.currentContext.clientBuffers[i];
      if (!cb.clientside || !cb.enabled) continue;
      GL.resetBufferBinding = true;
      let size = GL.calcBufLength(cb.size, cb.type, cb.stride, count);
      let buf = GL.getTempVertexBuffer(size);
  ...
  registerContext: function (ctx, webGLContextAttributes) {
    let handle = GL.getNewId(GL.contexts);
    let context = {
      handle: handle,
  ...
  initExtensions: function (context) {
    ...
    let exts = GLctx.getSupportedExtensions() || [];
...
function _glBindVertexArray(vao) {
  ...
  let ibo = GLctx.getParameter(34965);
...
function _glDeleteBuffers(n, buffers) {
  for (let i = 0; i < n; i++) {
    let id = SAFE_HEAP_LOAD(buffers + i * 4 | 0, 4, 0) | 0;
...
function _glDeleteFramebuffers(n, framebuffers) {
  for (let i = 0; i < n; ++i) {
    let id = SAFE_HEAP_LOAD(framebuffers + i * 4 | 0, 4, 0) | 0;
...
function _glDeleteRenderbuffers(n, renderbuffers) {
  for (let i = 0; i < n; i++) {
    let id = SAFE_HEAP_LOAD(renderbuffers + i * 4 | 0, 4, 0) | 0;
    let renderbuffer = GL.renderbuffers[id];
...
function _glDeleteTextures(n, textures) {
  for (let i = 0; i < n; i++) {
    let id = SAFE_HEAP_LOAD(textures + i * 4 | 0, 4, 0) | 0;
...
function _glDeleteVertexArrays(n, vaos) {
  for (let i = 0; i < n; i++) {
    let id = SAFE_HEAP_LOAD(vaos + i * 4 | 0, 4, 0) | 0;
...
function _glDisableVertexAttribArray(index) {
  let cb = GL.currentContext.clientBuffers[index];
  cb.enabled = false;
...
let tempFixedLengthArray = [];

function _glDrawBuffers(n, bufs) {
  let bufArray = tempFixedLengthArray[n];
  for (let i = 0; i < n; i++) {
...
function _glDrawElements(mode, count, type, indices) {
  ...
  if (!GLctx.currentElementArrayBufferBinding) {
    let size = GL.calcBufLength(1, type, 0, count);
...
function __glGenObject(n, buffers, createFunction, objectTable) {
  for (let i = 0; i < n; i++) {
    let buffer = GLctx[createFunction]();
...
function __glGetActiveAttribOrUniform(funcName, program, index, bufSize, length, size, type, name) {
  ...
  let info = GLctx[funcName](program, index);
  if (info) {
    let numBytesWrittenExclNull = name && stringToAddress(info.name, name, bufSize);
...
function writeI53ToI64(ptr, num) {
  ...
  let deserialized = num >= 0 ? readI53FromU64(ptr) : readI53FromI64(ptr);
...
function emscriptenWebGLGet(name_, p, type) {
  ...
  switch (name_) {
    ...
    case 34466:
      let formats = GLctx.getParameter(34467);
  ...
  if (ret === undefined) {
    let result = GLctx.getParameter(name_);
...
function _glGetError() {
  let error = GLctx.getError() || GL.lastError;
  GL.lastError = 0;
  return error;
}
...
function _glGetProgramInfoLog(program, maxLength, length, infoLog) {
  ...
  let numBytesWrittenExclNull = maxLength > 0 && infoLog ? stringToAddress(log, infoLog, maxLength) : 0;
  if (length) SAFE_HEAP_STORE(length | 0, numBytesWrittenExclNull | 0, 4);
}
...
function _glGetProgramiv(program, pname, p) {
  ...
  if (pname == 35716) {
    ...
    if (!program.maxUniformLength) {
      for (let i = 0; i < GLctx.getProgramParameter(program, 35718); ++i) {
    ...
    if (!program.maxAttributeLength) {
      for (let i = 0; i < GLctx.getProgramParameter(program, 35721); ++i) {
    ...
    if (!program.maxUniformBlockNameLength) {
      for (let i = 0; i < GLctx.getProgramParameter(program, 35382); ++i) {
...
function _glGetShaderSource(shader, bufSize, length, source) {
  let result = GLctx.getShaderSource(GL.shaders[shader]);
  if (!result) return;
  let numBytesWrittenExclNull = bufSize > 0 && source ? stringToAddress(result, source, bufSize) : 0;
  if (length) SAFE_HEAP_STORE(length | 0, numBytesWrittenExclNull | 0, 4);
}
...
function _glGetShaderiv(shader, pname, p) {
  ...
  if (pname == 35716) {
    ...
    let source = GLctx.getShaderSource(GL.shaders[shader]);
...
function _glGetString(name_) {
  ...
  if (!ret) {
    switch (name_) {
      ...
      case 37445:
      case 37446:
        let s = GLctx.getParameter(name_);
        if (!s) {
          GL.recordError(1280);
        }
      ...
      case 7938:
        let glVersion = GLctx.getParameter(7938);
      ...
      case 35724:
        let glslVersion = GLctx.getParameter(35724);
...
function _glGetStringi(name, index) {
  ...
  let stringiCache = GL.stringiCache[name];
  if (stringiCache) {
...
function webglPrepareUniformLocationsBeforeFirstUse(program) {
  let uniformLocsById = program.uniformLocsById, uniformSizeAndIdsByName = program.uniformSizeAndIdsByName, i, j;
  if (!uniformLocsById) {
    program.uniformLocsById = uniformLocsById = {};
    program.uniformArrayNamesById = {};
    for (i = 0; i < GLctx.getProgramParameter(program, 35718); ++i) {
      let u = GLctx.getActiveUniform(program, i);
      let nm = u.name;
      let sz = u.size;
      let lb = webglGetLeftBracePos(nm);
...
function _glGetUniformLocation(program, name) {
  ...
  if (program = GL.programs[program]) {
    ...
    let leftBrace = webglGetLeftBracePos(name);
    if (leftBrace > 0) {
      arrayIndex = jstoi_q(name.slice(leftBrace + 1)) >>> 0;
      uniformBaseName = name.slice(0, leftBrace);
    }
    let sizeAndId = program.uniformSizeAndIdsByName[uniformBaseName];
...
function emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, internalFormat) {
  let heap = heapObjectForWebGLType(type);
  let shift = heapAccessShiftForWebGLHeap(heap);
  let byteSize = 1 << shift;
  let sizePerPixel = __colorChannelsInGlTextureFormat(format) * byteSize;
  let bytes = computeUnpackAlignedImageSize(width, height, sizePerPixel, GL.unpackAlignment);
...
function _glReadPixels(x, y, width, height, format, type, pixels) {
  ...
  let pixelData = emscriptenWebGLGetTexPixelData(type, format, width, height, pixels, format);
  if (!pixelData) {
    GL.recordError(1280);
    return;
  }
